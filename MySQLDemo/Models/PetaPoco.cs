...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        //zjl add
        public IDbCommand CreateProcCommand(IDbConnection connection, string sql, params IDbDataParameter[] args)
        {
        ...
        #region SprocQuery
        public IEnumerable<T> SprocQuery<T>(string sql, params IDbDataParameter[] args)
        {
        ...
        public List<T> SprocFetch<T>(string ProcName, params IDbDataParameter[] args)
        {
            return SprocQuery<T>(ProcName, args).ToList();
        }
        public int SprocExecute(string ProcName, params IDbDataParameter[] args)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateProcCommand(_sharedConnection, ProcName, args))
                    {
                        var retv = cmd.ExecuteNonQuery();
                        OnExecutedCommand(cmd);
                        return retv;
        ...
        {
            return FormatCommand(cmd.CommandText, (from IDataParameter parameter in cmd.Parameters select parameter).ToArray());
        }
        ...
        /// <returns></returns>
        public string FormatCommand(string sql, IDataParameter[] args)
        {
            var sb = new StringBuilder();
            if (sql == null)
                return "";
            ...
            if (args != null && args.Length > 0)
            {
                ...
                {
                    sb.AppendFormat("\t -> {0} [DBType:{1} {2}] = \"{3}\"\n",
                        args[i].ParameterName,
                        Enum.GetName(typeof(DbType), args[i].DbType),
                        Enum.GetName(typeof(ParameterDirection), args[i].Direction),
                        args[i].Value ?? "null"
                        );
                }
        ...
        internal void DoPreExecute(IDbCommand cmd)
        {
            ...
            _lastArgs = (from IDataParameter parameter in cmd.Parameters select parameter).ToArray();
        }
